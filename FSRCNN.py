...
Y_ = tf.nn.sigmoid(tf.nn.conv2d_transpose(z_Exp,W_Deconv, output_shape, strides=[1,2,2,1], padding='SAME'))
...
#                    train_op = tf.train.AdamOptimizer(learning_rate).minimize(MSE)
                ...
                train_pSNR = 10*math.log(1/train_MSE,10)
                train_SSIM = -1
...
#test_batch_size = TEST_BATCH_SIZE
    ...
    with tf.Session() as sess:
        ...
        #train_pSNR = pSNR.eval(feed_dict={img_LR:img_LR_, img_HR:img_HR_, keep_prob: 1.0})
        test_pSNR = 10*math.log(1/test_MSE,10)
        test_SSIM = -1
