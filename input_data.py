...
'''
#%%
def image_simple_preprocess(img):
    img = img*2.0 /255 - 1
    return
def image_simple_recover(img):
    img = (img + 1)*255/2
    return

def convert_to_tfrecord(dataset_name, save_dir = "TFRecord_DataSet/", label='train' ,HR_size = HR_SIZE, LR_size = LR_SIZE, LR_downsample_size=LR_downsample_SIZE):
...
#        img_path = '/Users/Kingther/百度云同步盘/【★快盘★】/【毕业设计】/DataSet/San11/0000_杨奉_1.jpg'
        try:
            ...
            img_LR_downsample = img_HR.resize(LR_downsample_size)
            if LR_downsample_size==LR_size:
                img_LR = img_LR_downsample
            else:
                img_LR = img_LR_downsample.resize(LR_size,interpolation)

            img_HR_raw = np.array(img_HR).tobytes()#将图片转化为二进制格式
            img_LR_raw = np.array(img_LR).tobytes()#将图片转化为二进制格式

            example = tf.train.Example(features=tf.train.Features(feature={
                'img_HR_raw': tf.train.Feature(bytes_list=tf.train.BytesList(value=[img_HR_raw])),
                'img_LR_raw': tf.train.Feature(bytes_list=tf.train.BytesList(value=[img_LR_raw]))
...
def read_and_decode(tfrecords_filename,HR_size = HR_SIZE, LR_size = LR_SIZE, LR_downsample_size=LR_downsample_SIZE):
    ...
    features = tf.parse_single_example(
                                        ...
                                        features={
                                           ...
                                           'img_LR_raw' : tf.FixedLenFeature([], tf.string)})
    img_HR = tf.decode_raw(features['img_HR_raw'], tf.uint8)
    img_LR = tf.decode_raw(features['img_LR_raw'], tf.uint8)
    ...
    img_LRs = tf.reshape(img_LR, [LR_size[0], LR_size[1], 3])  
    #img_HR = tf.cast(img_HR, tf.float32) * (1. / 255) - 0.5 
    #img_LR = tf.cast(img_LR, tf.float32) * (1. / 255) - 0.5  

  

    return img_HRs, img_LRs

#%%
def get_batch(img_HRs, img_LRs,batch_size= BATCH_SIZE,num_threads=32,shuffle = True):
    if shuffle == True:
        img_HR_batch, img_LR_batch = tf.train.shuffle_batch([img_HRs, img_LRs],
                                            batch_size= batch_size,
                                            num_threads= num_threads, 
                                            min_after_dequeue = 100,
                                            capacity = 1000)
    else:
        img_HR_batch, img_LR_batch = tf.train.batch([img_HRs, img_LRs],
                                            batch_size= batch_size,
                                            num_threads= num_threads,
                                            allow_smaller_final_batch = True,
                                            capacity = 1000)    
    return img_HR_batch, img_LR_batch
