...
train_dataset_label='train_L120H240'
test_dataset_label='test_L120H240'
...
input_height = HR_size[0]
input_width = HR_size[1]
input_channel = 3
output_height = LR_size[0]
output_width = LR_size[1]
output_channel = 3
#%%
# CNN Hyperparameters
step_save = 200
...
img_SR = mytf.conv2d(z_conv2_dropout, W_conv3) + b_conv3
...
##############################################
def train(step_save=step_save,step_train=2000,train_continue = False):
...
#%% For Debug
'''
img_HRs_test, img_LRs_test = input_data.read_and_decode(train_tfrecords_filename,HR_size = HR_size, LR_size = LR_size, LR_downsample_size=LR_downsample_size)
...
'''
'''
def debug2(test_batch_size = TEST_BATCH_SIZE):
    saver = tf.train.Saver(tf.trainable_variables())
    img_HRs_test, img_LRs_test = input_data.read_and_decode(test_tfrecords_filename,HR_size = HR_size, LR_size = LR_size, LR_downsample_size=LR_downsample_size)
    ...
    with tf.Session() as sess:
        ...
        coord.request_stop()#queue需要关闭，否则报错  
        coord.join(threads)
        mytf.plot_imagepairs(img_LR_test,img_HR_test,num_img = test_batch_size)
        return img_LR_test,img_HR_test
'''